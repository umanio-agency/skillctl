...
- `[~]` **Phase 6 — Polish & open source.** README usage section and CI (lint + test) shipped 2026-05-03 as Phase 6a; help text and error messages were already cleaned up incrementally during earlier phases. The **public flip remains gated**: per 2026-05-02 user directive, repo stays private until the tool has been used for real on multiple projects and the remaining backlog items the user wants land first. Don't propose flipping public until the user explicitly says they're ready.
...
- **2026-05-03** — **Custom prompt disables terminal line-wrap during the run.** First test surfaced a real bug: long footer text wrapped onto a second visual line, but `last_lines` only counted the logical lines we queued, so `clear_render` moved up too few rows and the prompt header accumulated on every keypress. Fix: wrap the run in `DisableLineWrap` / `EnableLineWrap` (crossterm) so `\r\n` always advances exactly one physical line, and shorten the footer to leave headroom regardless of terminal width. Lesson for future TUI work: never trust visual line counts; either disable wrap or measure rendered cells.
- **2026-05-03** — **CI is `.github/workflows/ci.yml` on ubuntu-latest, single job.** Steps: `cargo fmt --all -- --check`, `cargo clippy --all-targets -- -D warnings`, `cargo build --all-targets`, `cargo test --all`. Triggers on push to `main` and on every pull request. `dtolnay/rust-toolchain@stable` + `Swatinem/rust-cache@v2` for toolchain and dep caching. macOS/Windows matrix deferred — uniform build is enough as a regression net for now; expand if a platform-specific issue surfaces. `RUSTFLAGS=-D warnings` is set globally so any warning fails CI even if it slips past clippy.
- **2026-05-03** — **`cargo fmt` was run once and folded into the CI baseline.** Before Phase 6a the project hadn't been auto-formatted — `cargo fmt --check` flagged ~10 files. The reformat is part of Phase 6a's commit; future commits must keep `cargo fmt --check` clean since CI now enforces it.
- **2026-05-03** — **README is the public-facing pitch + quick start; the agent contract lives in `skills-cli-usage`.** README has Why / What it does / Install / Quick start / Commands / Tags / Non-interactive / Development. It links to `.claude/skills/skills-cli-usage/SKILL.md` rather than duplicating the full flag matrix. Keeps two single-sources-of-truth: README for humans evaluating the tool, companion skill for agents driving it.
- **2026-05-04** — **`skills detect` library destination prompt now lists "Library root" as the first option.** Reason: many libraries (including the user's own `umanio-agency/skills-library`) use a flat layout where each skill is a top-level folder, not nested under `skills/`. Previously the prompt only offered "skills" or "Custom path…", forcing the user to type `.` every time. Now the order is: Library root → existing `skills/` folders (or the `skills/` preset if none) → Custom. The default-selected option (cliclack picks the first) is therefore "Library root", matching the most common case. The non-interactive `--target .` already worked; the `is_root` helper in `commands/detect.rs` keeps `lib_relative` clean (no leading `./`) and normalises the JSON `target` field to `.`.
