...
> **Backlog (post-v1, see §10):** skill tags, progressive-filter multi-select prompt, pull library updates into installed skills.
>
> *Already shipped from the backlog (early):* §10.3 non-interactive agent mode (2026-05-01). See Phase 5.5 above.
...
- **Pull library updates into installed skills?** When the library has moved past a `.skills.toml` entry's `source_sha` and the local copy is `Unchanged`, `push` correctly says "no local changes" — but there's no way to **pull** the upstream improvements into the project. A future `skills update` (or `skills pull`) command would close that gap. Documented as a backlog item under §10.4.
